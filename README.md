...
- **Agent (Gemini):** `POST /v1/agent/step?session_id=<uuid>` with an **empty body** — no second image upload. Uses the latest WebP + detections from that session. Optional query params: `goal`, `last_outcome`. Requires **`GEMINI_API_KEY`** on the server. Returns JSON `{ say, actions, state_version, task_anchor }` (`task_anchor` is the session headline after this step: empty means exploration mode). **409** if no frames received yet; **429** if throttled or a call is already in flight; **503** if the API key is missing.
